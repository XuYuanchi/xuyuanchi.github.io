  ...
  return entries
    .map(
      (
        ...
        // Parse tags/keywords
        const keywords =
          tags.keywords?.split(",").map((k: string) => k.trim()) || [];
        ...
        const publication: Publication = {
          ...
          authors,
          year,
          month: monthMapping[tags.month?.toLowerCase()]
            ? String(month)
            : tags.month,
          type,
          status: "published",
...
function parseAuthors(
  ...
  return authorsStr
    ...
    .map((author) => {
      ...
      const isHighlighted =
        name.toLowerCase().includes("yanchi su") ||
        name.toLowerCase().includes("su yanchi");
